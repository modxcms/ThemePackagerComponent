...
TP.grid.Plugins = function(config) {
    ...
    Ext.applyIf(config,{
        ...
        ,tbar: [{
            text: _('themepackagercomponent.plugin_add')
...
Ext.extend(TP.grid.Plugins,TP.grid.LocalGrid,{
    getMenu: function() {
        return [{
            text: _('themepackagercomponent.plugin_remove')
            ,handler: this.remove.createDelegate(this,[{
                title: _('themepackagercomponent.plugin_remove')
                ,text: _('themepackagercomponent.plugin_remove_confirm')
            }])
            ,scope: this
        }];
    }
...
TP.window.AddPlugin = function(config) {
    ...
    Ext.applyIf(config,{
        title: _('themepackagercomponent.plugin_add')
        ,frame: true
        ,id: 'tp-window-plugin-add'
        ,fields: [{
            xtype: 'tp-combo-plugin'
            ,fieldLabel: _('themepackagercomponent.plugin')
            ,description: _('themepackagercomponent.plugin_desc')
            ,name: 'plugin'
            ,hiddenName: 'plugin'
...
Ext.extend(TP.window.AddPlugin,MODx.Window,{
    submit: function() {
        ...
        if (id != '' && this.fp.getForm().isValid()) {
            ...
            MODx.msg.alert(_('error'),_('themepackagercomponent.plugin_err_ns'));
        }
    }
});
