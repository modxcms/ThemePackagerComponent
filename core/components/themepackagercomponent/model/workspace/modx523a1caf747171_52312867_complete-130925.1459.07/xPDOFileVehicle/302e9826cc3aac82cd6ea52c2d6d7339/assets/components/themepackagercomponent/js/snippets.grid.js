...
TP.grid.Snippets = function(config) {
    ...
    Ext.applyIf(config,{
        ...
        ,autoHeight: true
        ,columns: [{
            header: _('themepackagercomponent.snippet')
            ,dataIndex: 'name'
            ,width: 250
        },{
            header: _('themepackagercomponent.assets_path')
            ,dataIndex: 'assets_path'
            ,width: 300
        },{
            header: _('themepackagercomponent.core_path')
            ,dataIndex: 'core_path'
            ,width: 300
        }]
        ,tbar: [{
            text: _('themepackagercomponent.snippet_add')
...
Ext.extend(TP.grid.Snippets,TP.grid.LocalGrid,{
    getMenu: function() {
        return [{
            text: _('themepackagercomponent.snippet_remove')
            ,handler: this.remove.createDelegate(this,[{
                title: _('themepackagercomponent.snippet_remove')
                ,text: _('themepackagercomponent.snippet_remove_confirm')
            }])
            ,scope: this
        }];
    }
...
TP.window.AddSnippet = function(config) {
    ...
    Ext.applyIf(config,{
        title: _('themepackagercomponent.snippet_add')
        ,frame: true
        ,id: 'tp-window-snippet-add'
        ,fields: [{
            xtype: 'tp-combo-snippet'
            ,fieldLabel: _('themepackagercomponent.snippet')
            ,description: _('themepackagercomponent.snippet_desc')
            ,name: 'snippet'
            ,hiddenName: 'snippet'
            ...
            ,fieldLabel: _('themepackagercomponent.assets_path')
            ,description: _('themepackagercomponent.assets_path_desc')
            ...
            ,fieldLabel: _('themepackagercomponent.core_path')
            ,description: _('themepackagercomponent.core_path_desc')
...
Ext.extend(TP.window.AddSnippet,MODx.Window,{
    submit: function() {
        ...
        if (id != '' && this.fp.getForm().isValid()) {
            ...
            MODx.msg.alert(_('error'),_('themepackagercomponent.snippet_err_ns'));
        }
    }
