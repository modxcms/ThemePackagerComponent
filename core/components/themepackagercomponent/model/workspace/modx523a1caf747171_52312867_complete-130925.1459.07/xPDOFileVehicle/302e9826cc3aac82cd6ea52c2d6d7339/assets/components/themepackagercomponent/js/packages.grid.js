...
TP.grid.Packages = function(config) {
    ...
    Ext.applyIf(config,{
        ...
        ,autoHeight: true
        ,columns: [{
            header: _('themepackagercomponent.signature')
            ,dataIndex: 'signature'
            ,width: 250
        }]
        ,tbar: [{
            text: _('themepackagercomponent.subpackage_add')
...
Ext.extend(TP.grid.Packages,TP.grid.LocalGrid,{
    getMenu: function() {
        return [{
            text: _('themepackagercomponent.subpackage_remove')
            ,handler: this.remove.createDelegate(this,[{
                title: _('themepackagercomponent.subpackage_remove')
                ,text: _('themepackagercomponent.subpackage_remove_confirm')
            }])
            ,scope: this
        }];
    }
...
TP.window.AddPackage = function(config) {
    ...
    Ext.applyIf(config,{
        title: _('themepackagercomponent.subpackage_add')
        ,frame: true
        ,id: 'tp-window-package-add'
        ,fields: [{
            xtype: 'tp-combo-package'
            ,fieldLabel: _('themepackagercomponent.subpackage')
            ,description: _('themepackagercomponent.subpackage_desc')
            ,name: 'package'
            ,hiddenName: 'package'
...
Ext.extend(TP.window.AddPackage,MODx.Window,{
    submit: function() {
        ...
        if (id != '' && this.fp.getForm().isValid()) {
            ...
            MODx.msg.alert(_('error'),_('themepackagercomponent.subpackage_err_ns'));
        }
    }
});
