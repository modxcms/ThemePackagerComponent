...
TP.grid.Templates = function(config) {
    ...
    Ext.applyIf(config,{
        ...
        ,autoHeight: true
        ,columns: [{
            header: _('themepackagercomponent.template')
            ,dataIndex: 'name'
            ,width: 250
        },{
            header: _('themepackagercomponent.directory')
            ,dataIndex: 'directory'
            ,width: 400
        }]
        ,tbar: [{
            text: _('themepackagercomponent.template_add')
...
Ext.extend(TP.grid.Templates,TP.grid.LocalGrid,{
    getMenu: function() {
        return [{
            text: _('themepackagercomponent.template_remove')
            ,handler: this.remove.createDelegate(this,[{
                title: _('themepackagercomponent.template_remove')
                ,text: _('themepackagercomponent.template_remove_confirm')
            }])
            ,scope: this
        }];
    }
...
TP.window.AddTemplate = function(config) {
    ...
    Ext.applyIf(config,{
        title: _('themepackagercomponent.template_add')
        ,frame: true
        ,id: 'tp-window-template-add'
        ,fields: [{
            xtype: 'tp-combo-template'
            ,fieldLabel: _('themepackagercomponent.template')
            ,description: _('themepackagercomponent.template_desc')
            ,name: 'template'
            ,hiddenName: 'template'
            ...
            xtype: 'textfield'
            ,name: 'directory'
            ,fieldLabel: _('themepackagercomponent.directory')
            ,description: _('themepackagercomponent.template_directory_desc')
...
Ext.extend(TP.window.AddTemplate,MODx.Window,{
    submit: function() {
        ...
        if (id != '' && this.fp.getForm().isValid()) {
            ...
            MODx.msg.alert(_('error'),_('themepackagercomponent.template_err_ns'));
        }
    }
