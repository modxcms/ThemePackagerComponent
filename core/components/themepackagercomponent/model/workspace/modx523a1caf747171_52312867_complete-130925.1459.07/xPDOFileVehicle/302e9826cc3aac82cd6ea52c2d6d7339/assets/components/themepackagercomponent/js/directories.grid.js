...
TP.grid.Directories = function(config) {
    ...
    Ext.applyIf(config,{
        ...
        ,autoHeight: true
        ,columns: [{
            header: _('themepackagercomponent.directory_source')
            ,dataIndex: 'source'
            ,width: 300
        },{
            header: _('themepackagercomponent.directory_target')
            ,dataIndex: 'target'
            ,width: 300
        }]
        ,tbar: [{
            text: _('themepackagercomponent.directory_add')
...
Ext.extend(TP.grid.Directories,TP.grid.LocalGrid,{
    getMenu: function() {
        return [{
            text: _('themepackagercomponent.directory_remove')
            ,handler: this.remove.createDelegate(this,[{
                title: _('themepackagercomponent.directory_remove')
                ,text: _('themepackagercomponent.directory_remove_confirm')
            }])
            ,scope: this
        }];
    }
...
TP.window.AddDirectory = function(config) {
    ...
    Ext.applyIf(config,{
        title: _('themepackagercomponent.directory_add')
        ,frame: true
        ,id: 'tp-window-directory-add'
        ,width: 500
        ,fields: [{
            html: _('themepackagercomponent.directory_add_desc')+'<br />'
        },{
            xtype: 'textfield'
            ,name: 'source'
            ,id: 'tp-'+this.ident+'-source'
            ,fieldLabel: _('themepackagercomponent.directory_source')
            ,description: _('themepackagercomponent.directory_source_desc')
            ,value: ''
            ,allowBlank: false
            ,width: 300
        },{
            ...
            ,fieldLabel: _('themepackagercomponent.directory_target')
            ,description: _('themepackagercomponent.directory_target_desc')
            ,value: ''
            ,allowBlank: false
            ,width: 300
        }]
...
Ext.extend(TP.window.AddDirectory,MODx.Window,{
    submit: function() {
        ...
        if (this.fp.getForm().isValid()) {
            ...
            MODx.msg.alert(_('error'),_('themepackagercomponent.directory_err_ns'));
        }
    }
});
