...
TP.grid.Chunks = function(config) {
    ...
    Ext.applyIf(config,{
        ...
        ,tbar: [{
            text: _('themepackagercomponent.chunk_add')
...
Ext.extend(TP.grid.Chunks,TP.grid.LocalGrid,{
    getMenu: function() {
        return [{
            text: _('themepackagercomponent.chunk_remove')
            ,handler: this.remove.createDelegate(this,[{
                title: _('themepackagercomponent.chunk_remove')
                ,text: _('themepackagercomponent.chunk_remove_confirm')
            }])
            ,scope: this
        }];
    }
...
TP.window.AddChunk = function(config) {
    ...
    Ext.applyIf(config,{
        title: _('themepackagercomponent.chunk_add')
        ,frame: true
        ,id: 'tp-window-chunk-add'
        ,fields: [{
            xtype: 'tp-combo-chunk'
            ,fieldLabel: _('themepackagercomponent.chunk')
            ,description: _('themepackagercomponent.chunk_desc')
            ,name: 'chunk'
            ,hiddenName: 'chunk'
...
Ext.extend(TP.window.AddChunk,MODx.Window,{
    submit: function() {
        ...
        if (id != '' && this.fp.getForm().isValid()) {
            ...
            MODx.msg.alert(_('error'),_('themepackagercomponent.chunk_err_ns'));
        }
    }
});
