<?php
namespace Siphon\Transport;

class SiphonTransport extends \xPDOTransport {
    /**
    ...
    public static function retrieve(& $xpdo, $source, $target, $state= self::STATE_PACKED) {
        ...
        if (file_exists($source)) {
            if (is_writable($target)) {
                ...
                if ($manifest) {
                    $instance = new SiphonTransport($xpdo, $signature, $target);
                    if (!$instance) {
                        $xpdo->log(\xPDO::LOG_LEVEL_ERROR, "Could not instantiate a valid SiphonTransport object from the package {$source} to {$target}. SIG: {$signature} MANIFEST: " . print_r($manifest, 1));
                    }
